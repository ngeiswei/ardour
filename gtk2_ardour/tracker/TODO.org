- Try to understand segfault when loading test-3
- Deal with *** in a "proper" manner...
- Support arbitrary number of tracks (put all properties in tracker_column)
- Fix bug in updating playhead
- Fix alignment with track toolbar
- Fix grid focus. When click on a button, bring focus immediately back to the grid.
- Fix window focus (the tracker editor window should have focus right away)
- Support Gtk::Expander to grid header (see cpuload_expander)
- Try to replace as many gtk widgets by cairo widgets
- Take care of code style
- Try to improve color usage, how to label them, what can be reused
- const when possible
- Create wscript under tracker folder
- Underline the digit to be edited according to position
- Support shift-tab to move from right to left
- Add shortcut for parameters, steps, etc
- Add copy/move, etc, notes + popup menu
- Transfer Ardour shortcuts, spacebar, etc
- Support layered regions
- Add piano keyboard display (see gtk_pianokeyboard.h)
- Support midi input
- Make virtual keyboard keys not hardcoded
- Only have Octave, Ch, Vel in main toolbar is there is a midi track, maybe
- Play note velocity at which it is gonna be recorded (use already existing)
- Use signals whenever possible (instead of for instance calling
  Grid::redisplay_model) to not block. This can make ardour crash when jack
  buffer size is too short.
- Fix updating grib when note or automations are updated in the editor
- Fix row per beat fix when not the first tracker editor instance (somehow the
  signal must be caught by the first instance instead of the second). One way
  to fix that would be to only allow one instance of the tracker editor to be
  active at any given time. So that it's not possible to open 2 tracker editor
  at the same time, and when someone closes the current tracker editor it gets
  destructed.
